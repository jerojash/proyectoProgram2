...
struct vehiculo{                                     //ESTRUCTURA FECHA
	...
	struct fecha annio;                                //A√ëO DEL VEHICULO
...
void encabezado();                                   //ENCABEZADO PARA MOSTRAR EN LOS MEN√öS
...
<<<<<<< HEAD
	...
	printf("\n\t\t\tIngrese el aÒo en que se registro la infraccion: "); 
	...
	while ((strcmp(auxInfraccion->pagado,"SI")!=0)||(strcmp(auxInfraccion->pagado,"NO")!=0)){   //******************VALIDACION SI PAGO MULTA O NO********************
		...
		printf("\n\n\t\t\tIngrese el aÒo en que se registro la infraccion: %i",auxInfraccion->fechaInfraccion.yy);
		printf("\n\n\t\t\tIngrese si la multa ha sido pagada (SI/NO): "); 
		gets(auxInfraccion->pagado);
	}
